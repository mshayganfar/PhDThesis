...
\ref{sec:appraisal}. We describe the methods which retrieve information about
...
\begin{figure}[tbh]
  ...
  \caption{{\fontsize{10}{10}\selectfont Using Collaboration structure in
...
\begin{figure}[t]
  ...
  \caption{{\fontsize{10}{10}\selectfont Using Appraisals' outcome to influence
  Collaboration structure (mechanisms in our framework).}}
...
structure (see Fig. \ref{fig:appraisal-on-collaboration}).
\vspace*{3mm}
...
\begin{figure}[tbh]
  ...
  \caption{{\fontsize{10}{10}\selectfont Astronaut-robot collaboration structure
  (shared plan).}}
...
the other collaborator's emotion is \textit{negative} \textbf{and} the valance
of the robot's emotion is also \textit{negative}, the active coping, the
acceptance, and the mental disengagement coping strategies are the coping
strategy candidates that are going to become activated if the other activation
criteria also exist for any of them. The secons criterion represent the need to
cope with an event in a certain way. The values of our three different motives
(i.e., \textit{satisfaction}, \textit{achievement}, and \textit{external}) are
involved in the decision of whether there is a need for a particular coping
strategy to become activated. We use conjunction of satisfaction motive's
value with the disjunction of achievement and external motives. For instance, if
we have highly negative values for all three motives for the candidate coping
strategies in our previous example, the acceptance strategy will be selected as
the strategy with highest need. However, the ability to cope with an event as
the third criterion impacts the decide of whether the selected coping startegy
can be activated. The controllability of an event indicates whether the agent
is able to cope with the given event. In this example, if the agent finds the
...
\section{Using Appraisal in Collaboration}
