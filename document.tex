...
\begin{enumerate}
  ...
  (Chapter \ref{ch:appraisals}) In order to evaluate our computational models and
  ...
  system evaluation with participants and a robot. The second user study was
  also conducted to evaluate the benefit of using our computational framework in
  human-robot collaboration. In the first user study, we crowd sourced our
  questionnaires to test our hypothesis that humans and our algorithms will
  provide similar answers to questions related to different factors within our
  appraisal algorithms. In the second user study, we investigated the importance
  of emotional awareness in human-robot collaboration, and the overall
...
\vspace*{-3mm}
In this chapter, we review the background of prominent collaboration theories
including the the SharedPlans theory \cite{grosz:plans-discourse} as one of the
two theoretical foundations of our work. We also provide the similarities and
differences between these theories as well as both theoretical and practical
related work and applications of them. We continue by discussing the concept of
...
important in our work, since it is focused on collaboration which is a social
phenomenon in human environments. We also present the concept of artificial
emotions and provide some examples of the existing computational models of
emotions. Then, we provide the background of the cognitive appraisal theory
of emotions as the second theoretical foundation in our work as well as other
computational models of emotions and the related concepts such as some examples
of cognitive architectures and the influence of affect in decision-making
procedures. This chapter continues with the description of motives and the
related theories in psychology and artificial intelligence. The role of motives
as goal-driven affective components is crucial in our work, since the
collaboration structure is built based on the concept of a shared goal between
collaborators. Finally, a brief description and the related work in psychology
and artificial intelligence is provided about theory of mind as another concept
used in a small limited-scale in our work.
...
systems better at helping us do whatever they are designed for. To build
...
\cite{grosz:shared-plans, lochbaum:collaborative-planning}. This section briefly
provides descriptions of major computational collaboration theories, their
similarities and differences, and their application in AI and robotics. It
primarily focuses on Joint Intention, SharedPlans and hybrid theories of
collaboration. In this section, we do not present the theories in formal
...
\cite{Litman:discourse-commonsense}, and they were derived from the
Belief-Design-Intention (BDI) paradigm developed by Bratman
...
used extensively to examine and describe teamwork and collaboration.
...
this section we exaplain each of the mechanisms and the Mental States.
...
private beliefs (the agent believes the human does not know these), b) the inferred
...
\begin{itemize}
  ...
  regularly it appears as the result of the occurence of an event. The frequency
  of beliefs can impact attributes of the self's other Mental States. For
  instance, beliefs forming or maintaining intentions with direct experiences
  (see Section \ref{section-intention-attributes}) are more likely to occur
  frequently.
\end{itemize}
...
\section{Discussion}
This thesis work presents the Affective Motivational Collaboration Theory as the
theoretical foundation of our AMC framework which is built on the combination
of SharedPlans theory of collaboration \cite{grosz:plans-discourse} and
cognitive appraisal theory of emotions \cite{marsella:ema-process-model}
\cite{scherer:appraisal-processes}.The AMC framework is motivated by
the lack of a theory describing the associated processes involved in a dyadic
collaboration as well as their relationship and influences on each other. In
particular, in this thesis we emphesize on the reciprocal influence of
collaboration structure and the appraisal processes in collaboration. We provide
some algorithms and methods to compute appraisal variables and their influence
on collaboration processes, e.g. goal management, in this context. In general,
our contribution in this thesis is synthesizing prior work on appraisal and
collaboration to provide a theory which describes emotion-regulated goal-driven
behaviors within a dyadic collaboration. This thesis also makes note of the
influence of motives on coping process in collaboration. We also validate our
individual appraisal algorithms as well as our overall framework (end-to-end
system evaluation) by conducting an online crowd sourcing and an in-house
user studies respectively. The first user study is to investigate whether the
humans and our appraisal algorithms provide similar answers to the questions
(events) with respect to the change of particular factors invloved in our
algorithms. The second user study is to investigate a) importance of
emotional-awareness in collaboration, and b) the overall functionality of the
AMC framework to autonomously control interactions of a collaborative robot.

After an introduction, we presented the theoretical background on two major
foundations of our theory. First, we reviewed the prominent computational
...
between these theories in terms of their essential consepts as well as their
theoretical and practical applications. These applications involved fields of
robotic and artificial agents. Then, we continued with discussing what emotions
...
Later, we introduced our Affective Motivational Collaboration Theory. We
discussed all the mechanisms involved in our theory. All these mechanisms
include different functions each of which are designed to process and provide
a particular value required in overall operation of the system. Among these
mechanisms our focus was on Collaboration, Appraisal and Coping mechanisms.
Although other mechanisms such as Motivation play important role in influencing
the overall behavior of the agent using our framework. We also discussed the
events that we consider in a collaborative environment including utterance,
...
emotions and their influence on collaboration process. Therefore, we briefly
...
All of these Mental States can be defined and distinguish by their attributes.
...
In Chapter \ref{ch:appraisals}, we introduced the overall framwork using our AMC
theory in more details such as the main functions implemented in Collaboration
mechanism. As major part of this chapter, we explained our algorithms to compute
th values associated to the appraisal variables that we have implmeneted as part
of our framework. These algorithms are to compute the values of relevance,
desirability, expectedness and controllability of an event occurring during the
collaboration. All of these algorithms process the data provided by the
collaboration structure. Reciprocally, we also provided the datails of how we
use the outcome of the appraisals to influence the collaboration structure. We
introduced our algorithm for the goal management process. Then, we explained the
detail about the coping strategies, e.g. Active Coping, involved in our Coping
Mechanism and the underlying processes associated with these coping strategies.
This chapter continued on the details about the Motivation mechanism, the types
of motives we have implmeneted, their description and the way we compute the
values of these motives. We also briefly describe other mechanims in our
frmawork. Finally, we describe the elicitation of different emotion instances
in our framework and how they are interpreted according the different contexts
during collaboration.

\section{End Results}
With the above contributions, we end with a final user-study as our end-to-end
...
participants in a way that all of the participants prefer the robot with
emotional awareness running our framework.

In this user-study, we assigned participants to collaborate with the
...
first and then with the emotion-ignorant robot. All the participants
collaborated with the robot in both conditions. We asked participants to answer
our questionnaire with which we investigated the preferance of the participants
between two robots from different point of view. In general, all of the
participants prefered collaborating with the robot with emotional-awareness.
Also, in almost all of the imporatnt detailed factors, e.g. likability,
...
questionnaires. [WHAT CAN WE SAY HERE?]

\section{Future Work}

This work makes way for a number of potential extentions. In particular, we
believe that extentions to the system could be made by employing emotion
functions we discussed in Chapter \ref{ch:appraisals}. By acquiring these
emotion functions, one potential extention could be to begin exploring how
...
meaning of anger while the collaborators are negotiating pursuing or abondoning
a given goal. Since the meaning of the many of the social emotions are defined
in the field of psychology, the agent can use these meanings to improve the
likability or other factors improving the collaboration.
...
collaboration, e.g. trust or performance. In particular, an intersting extention
would be to enable the agent perceiving which aspect of the collaboration is
suffering and needs to be improved at any given time. For example, if the human
collaborator is loosing interest in achieving of a given goal, the agent can try
to motivate the human collaborator with the right decision and ultimately
behavior. This way the agent would be more adaptive duing collaboration.

Finally, in addition to expanding the adaptability of the agent to the human
collaborator's intrenal state, future extentions are also possible in the other
mechanisms as well. For example, an intersting area to explore is the ability to
expand and employ more elaborate computational models of motivation and theory
of mind. These extentions could provide more information about the human
...
learning abilities are also left as potential area for further exploration.
