...
valence value of the the human collaborator's perceived emotion.

\begin{equation}
    \mathcal{M}_{sat}(\varepsilon_t)= 
    \begin{dcases}
       arctan (1.5 \times \delta_{sat}) & valence = 0 \\
       3^{2 \times (\delta_{sat}-1)}    & valence > 0 \\
       -3^{-2 \times (\delta_{sat}+1)}  & valence < 0
    \end{dcases}
...
\textit{controllability} and \textit{expectedness} values. Intutively, the more
controllable and expected the events are, the probability of successful
